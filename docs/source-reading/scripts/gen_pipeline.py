...
stages = [
    ...
    ("Stream", "TCP 重组(UDP 跳过)", "#2a78d6", "#ffffff", "第6篇"),
    ("应用层解析", "HTTP / TLS ...", "#256abf", "#ffffff", "第7篇"),
    ("规则检测", "SGH + MPM", "#1c5cab", "#ffffff", "第8-9篇"),
...
parts.append(f'<text x="{W/2}" y="82" text-anchor="middle" font-size="15" fill="#52514e">一个包从网卡钻进来，依次经过这几道工位，最后以 EVE JSON 事件的形式输出</text>')
...
# source box
sx, sy = start_x, top_y
parts.append(rounded_box(sx, sy, source_w, box_h, "#e1e0d9", "流量来源", "网卡 / PCAP 文件", "#0b0b0b"))
...
fw_x = []  # Flow ~ 输出 五个工位都在 FlowWorker() 里
...
# FlowWorker 括线
bx1, bx2, by = fw_x[0], fw_x[-1] + box_w, top_y - 14
parts.append(f'<path d="M {bx1} {by+8} L {bx1} {by} L {bx2} {by} L {bx2} {by+8}" fill="none" stroke="#898781" stroke-width="1.5"/>')
parts.append(f'<text x="{(bx1+bx2)/2}" y="{by-8}" text-anchor="middle" font-size="13" font-weight="700" fill="#52514e">FlowWorker(同一个 TmModule)</text>')
