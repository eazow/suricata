...
parts.append(f'<rect x="0" y="0" width="{W}" height="{H}" fill="#fcfcfb"/>')
...
parts.append(label(tx+18, lane_y+22, "一个 ThreadVars（一个线程），驱动方式 pktacqloop", 12.5, SUB))
...
parts.append(label(tx+thread_w/2, lane_y+thread_h+22, "in / out 都是 packetpool，包全程不离开这个线程", 12, SUB, "middle"))

# ---- Section C: autofp lane ----
cy = lane_y + thread_h + 56
...
parts.append(label(gx1+18, lane2_y+22, "Receive 线程组，驱动方式 pktacqloop", 12.5, SUB))
s2y = lane2_y + 36
s2x = gx1 + 24
...
parts.append(box(qx, lane2_y, qw, qh, VIOLET, "pickup1..N", "按 p->flow_hash % N 分发", "#ffffff"))
parts.append(label(qx+qw/2, lane2_y+qh+20, "同一条流永远落进同一条队列", 12, SUB, "middle"))
...
parts.append(label(gx2+18, lane2_y+22, "Worker 线程组，驱动方式 varslot", 12.5, SUB))
s3y = lane2_y + 36
s3x = gx2 + 24
