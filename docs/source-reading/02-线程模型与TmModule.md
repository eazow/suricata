...
TmModule(说明书,全局唯一)
   │  TmSlotSetFuncAppend()
   ▼
TmSlot(上岗单,挂在某个 ThreadVars 的 tm_slots 链表上)
   │  slot_next 串成一条链
   ▼
TmThreadsSlotVarRun() 逐个调用 SlotFunc,包依次闯过每道工位

workers:  [ReceiveAFP → DecodeAFP → FlowWorker → RespondReject]   单线程全包,驱动方式 pktacqloop
single :  workers 的特例(线程数=1,网卡数=1)

autofp :  [ReceiveAFP → DecodeAFP] --按 flow_hash 分发--> pickupN 队列 --> [FlowWorker → RespondReject]
          第一组驱动方式 pktacqloop,第二组驱动方式 varslot,中间靠 Tmq + flow 哈希传包
```
